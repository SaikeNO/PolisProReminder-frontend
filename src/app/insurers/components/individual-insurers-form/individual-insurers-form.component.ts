...
import { CreateIndividualInsurer, IndividualInsurer } from '../../../shared/interfaces/insurer';
...
@Component({
  selector: 'app-individual-insurers-form',
  standalone: true,
  imports: [
    MatCardModule,
    MatInputModule,
  ...
  templateUrl: './individual-insurers-form.component.html',
  styleUrl: './individual-insurers-form.component.scss',
})
export class IndividualInsurersFormComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    @Inject(CONTAINER_DATA) public insurer: IndividualInsurer | undefined,
  ) {}
  ...
  public onSubmit() {
    ...
    const insurer = replaceEmptyStringWithNull(this.form.value) as CreateIndividualInsurer;

    if (this.insurer) {
      this.insurersFacade.editIndividualInsurer(insurer, this.insurer.id);
    } else {
      this.insurersFacade.createIndividualInsurer(insurer);
    }
  }
}
